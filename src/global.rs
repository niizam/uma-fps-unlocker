...
fn main() {
    ...
    let names = ["umamusumeprettyderby.exe"];
    let pid = match injector_wait::find_pid_with_wait(|| find_pid_by_names(&names), wait_seconds) {
        Some(pid) => pid,
        None => {
            if wait_seconds > 0 {
                eprintln!(
                    "Global process not found within {} seconds (umamusumeprettyderby.exe).",
                    wait_seconds
                );
            } else {
                eprintln!(
                    "Global process not found (umamusumeprettyderby.exe). Start the game first."
                );
            }
            std::process::exit(1);
        }
